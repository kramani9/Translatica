...
def get_transcripts_json(gcsPath, langCode, phraseHints=[], speakerCount=1, enhancedModel=None):
    ...
    # we are running into problems here
...
def dub(
    ...
    if not f"transcript.json" in outputFiles:
        ...
        print("Transcribing...")
        #its shitting the bed right here
    ...
    if not noTranslate:
        ...
        # we need to set up the OPEN AI api here
        for sentence in sentences:
            #sentence[lang] = translate_text(
                #sentence[srcLang], lang, srcLang)
            
            sentence[targetLangs] = translate_text_openai(
                sentence[srcLang], srcLang, targetLangs)
            #print("exec2")
    ...
    #for lang in targetLangs:
    languageDir = os.path.join(audioDir, targetLangs)
    ...
    for i, sentence in enumerate(sentences):
        ...
        audio = speakUnderDuration(
            sentence[targetLangs], targetLangs, sentence['end_time'] -
            sentence['start_time'],
            voiceName=voiceName)
    ...
    outFile = os.path.join(dubbedDir, targetLangs + ".mp4")
    stitch_audio(sentences, os.path.join(
        audioDir, targetLangs), videoFile, outFile, srtPath=srtPath)
